...
- Reach for the shared Playwright helpers rather than rebuilding them: `openNewTab`, `addWidget`, `openWidgetMenu`, `cardByTitle`, `boxOf`, `readWidgetSettings`, and `DEFAULT_BOARD_TITLES` in `e2e/helpers.ts`, and the spec-local `dragWidget`. A test that needs to assert mid-flow, or that drives the pointer at computed coordinates to check menu placement, should still write its steps out.
